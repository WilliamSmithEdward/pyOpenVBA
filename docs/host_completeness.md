...
| VBA WorksheetFunction and Evaluate | WorksheetFunction.X and the late-bound Application.X call the cell formula engine for its 75 functions, with arguments, answers and errors converted as 160 live calls made both ways show; Evaluate, `[...]` and Worksheet.Evaluate work out expressions as Excel does over 91 live cases: a Range for cells, blocks whole, error values | More functions in the engine; ROW()/COLUMN() without an argument as a one-item array |
