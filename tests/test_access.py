...
# OVBA-compressed into a single LVAL row. AccessReader.read_project_info()
...
def test_catalog_basic_project_metadata() -> None:
    from pyopenvba.access_read import AccessReader, AccessVBAProject

    db = AccessReader(_RE_CORPUS / "010__empty_StdModule_M.accdb")
...
def test_catalog_distinguishes_class_vs_standard_modules() -> None:
    ...
    db_class = AccessReader(_RE_CORPUS / "020__empty_ClassModule_C.accdb")
    ...
    db_std = AccessReader(_RE_CORPUS / "010__empty_StdModule_M.accdb")
...
def test_pcode_stream_lengths_match_corpus_baseline() -> None:
    ...
    for fname, expected_len in _PCODE_SAMPLES.items():
        ...
        if not path.exists():
            continue
        stream = AccessReader(path).read_module_pcode_stream()
...
def test_pcode_same_length_for_dim_variants_and_comment() -> None:
    ...
    lengths = {
        n: len(AccessReader(_RE_CORPUS / n).read_module_pcode_stream().raw)
        for n in names
    }
...
def test_pcode_type_token_differs_between_dim_integer_and_dim_long() -> None:
    ...
    a = AccessReader(_RE_CORPUS / "044__sub_dim_int.accdb").read_module_pcode_stream()
    b = AccessReader(_RE_CORPUS / "045__sub_dim_long.accdb").read_module_pcode_stream()
    assert len(a.raw) == len(b.raw)
    diff_offsets = [i for i, (x, y) in enumerate(zip(a.raw, b.raw, strict=True)) if x != y]
...
def test_pcode_invariant_string_literals_are_interned() -> None:
    ...
    a = AccessReader(_RE_CORPUS / "040__sub_msgbox_hello.accdb").read_module_pcode_stream()
    b = AccessReader(_RE_CORPUS / "041__sub_msgbox_world.accdb").read_module_pcode_stream()
    assert len(a.raw) == len(b.raw)
    diff = sum(1 for x, y in zip(a.raw, b.raw, strict=True) if x != y)
...
def test_pcode_immediate_int_literal_encoding() -> None:
    ...
    s = AccessReader(_RE_CORPUS / "048__sub_let_int_42.accdb").read_module_pcode_stream()
...
def test_pcode_immediate_true_encoded_as_push_i16_minus_one() -> None:
    ...
    s = AccessReader(_RE_CORPUS / "050__sub_if_true.accdb").read_module_pcode_stream()
...
def test_pcode_store_local_at_fbp_minus_8() -> None:
    ...
    for fname in ("047__sub_let_int.accdb", "048__sub_let_int_42.accdb"):
        s = AccessReader(_RE_CORPUS / fname).read_module_pcode_stream()
...
def test_pcode_for_loop_emits_for_init_and_step_opcodes() -> None:
    ...
    looped = AccessReader(_RE_CORPUS / "051__sub_for_1_to_3.accdb").read_module_pcode_stream()
    empty = AccessReader(_RE_CORPUS / "030__sub_A_empty.accdb").read_module_pcode_stream()
...
def test_pcode_comments_are_not_in_bytecode() -> None:
    ...
    empty = AccessReader(_RE_CORPUS / "030__sub_A_empty.accdb").read_module_pcode_stream()
    with_comment = AccessReader(_RE_CORPUS / "049__sub_comment_only.accdb").read_module_pcode_stream()
...
def test_pcode_local_vars_table_size_byte() -> None:
    ...
    def byte_at(fname: str, offset: int) -> int:
        return AccessReader(_RE_CORPUS / fname).read_module_pcode_stream().raw[offset]
...
def test_pcode_procedure_brackets_67_02_appear_in_every_sub() -> None:
    ...
    for fname in body_samples:
        raw = AccessReader(_RE_CORPUS / fname).read_module_pcode_stream().raw
...
def test_pcode_procedure_names_are_not_in_bytecode() -> None:
    ...
    raw_a = AccessReader(_RE_CORPUS / "030__sub_A_empty.accdb").read_module_pcode_stream().raw
    raw_b = AccessReader(_RE_CORPUS / "031__sub_B_empty.accdb").read_module_pcode_stream().raw
    raw_ab = AccessReader(_RE_CORPUS / "032__sub_AB_empty.accdb").read_module_pcode_stream().raw
...
def test_pcode_call_dispatch_is_literal_independent() -> None:
    ...
    raw_hello = AccessReader(
        _RE_CORPUS / "040__sub_msgbox_hello.accdb"
    ).read_module_pcode_stream().raw
    raw_world = AccessReader(
    ...
    diffs = [i for i, (a, b) in enumerate(zip(raw_hello, raw_world, strict=True)) if a != b]
...
def test_pcode_call_dispatch_block_marker_present() -> None:
    ...
    raw_call = AccessReader(
        _RE_CORPUS / "040__sub_msgbox_hello.accdb"
    ).read_module_pcode_stream().raw
    raw_empty = AccessReader(
...
def test_pcode_procend_close_form_present_in_every_sub() -> None:
    ...
    for name in bodies:
        if not (_RE_CORPUS / name).exists():
            continue
        raw = AccessReader(_RE_CORPUS / name).read_module_pcode_stream().raw
...
def test_pcode_trailer_cookie_is_deterministic_for_identical_bodies() -> None:
    ...
    * ``Sub A() / Dim x As Integer / End Sub`` (044)
    ...
    def cookie_of(path: Path) -> int:
        raw = AccessReader(path).read_module_pcode_stream().raw
...
def test_module_stream_contains_cafe_magic() -> None:
    ...
    for name in (
        ...
        if not path.exists():
            continue
        streams = AccessReader(path).find_module_streams()
...
def test_disassemble_empty_sub_yields_funcdefn_and_endsub() -> None:
    ...
    db = AccessReader(_RE_CORPUS / "030__sub_A_empty.accdb")
...
def test_disassemble_dim_recovers_dim_and_vardefn() -> None:
    ...
    db = AccessReader(_RE_CORPUS / "044__sub_dim_int.accdb")
    name = next(iter(db.iter_vba_modules())).name
    mod = db.disassemble_module(name)
    mnemonics = [
        ins.mnemonic
...
def test_disassemble_recovers_two_msgbox_calls_in_order() -> None:
    ...
    db = AccessReader(_RE_CORPUS / "043__sub_msgbox_two.accdb")
    name = next(iter(db.iter_vba_modules())).name
...
def test_disassemble_recovers_literal_42_assignment() -> None:
    ...
    db = AccessReader(_RE_CORPUS / "048__sub_let_int_42.accdb")
    name = next(iter(db.iter_vba_modules())).name
    flat = db.disassemble_module(name).iter_instructions()
...
def test_disassemble_recovers_if_endif_control_flow() -> None:
    ...
    db = AccessReader(_RE_CORPUS / "050__sub_if_true.accdb")
    name = next(iter(db.iter_vba_modules())).name
    mnemonics = {
        ins.mnemonic
        for ins in db.disassemble_module(name).iter_instructions()
...
def test_disassemble_recovers_for_next_control_flow() -> None:
    ...
    db = AccessReader(_RE_CORPUS / "051__sub_for_1_to_3.accdb")
    name = next(iter(db.iter_vba_modules())).name
    mnemonics = [
        ins.mnemonic
        for ins in db.disassemble_module(name).iter_instructions()
...
def test_identifiers_reflects_user_module_name_rename() -> None:
    ...
    db_m = AccessReader(_RE_CORPUS / "010__empty_StdModule_M.accdb")
    db_ab = AccessReader(_RE_CORPUS / "011__empty_StdModule_AB.accdb")
...
def test_identifiers_adds_user_variable_x_only_in_048() -> None:
    ...
    db030 = AccessReader(_RE_CORPUS / "030__sub_A_empty.accdb")
    db048 = AccessReader(_RE_CORPUS / "048__sub_let_int_42.accdb")
...
def test_identifiers_records_are_canonical_dataclass_instances() -> None:
    from pyopenvba.access_read import AccessReader, AccessVBAIdentifier
...
# ---------------------------------------------------------------------------
# Corpus parametrization helpers.
# ---------------------------------------------------------------------------
...
def test_msys_module_M_round_trip() -> None:
    from pyopenvba.access_read import MSYS_TYPE_MODULE, AccessReader
...
def test_pull_access_top_level(tmp_path: Path) -> None:
    """Top-level pull_access exports an Access database to a directory of .bas files."""
    import shutil
