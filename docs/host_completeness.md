...
| Formulas and calculation | Formula parser, dependency invalidation, manual mode and 122 registrations exist; a formula's last sum set to zero when it cancels, comparisons to fifteen digits and lookups to the bit follow Excel over 1,788 live pairs of doubles; defined names that stand for a formula work out where they are used; the range operator, intersections, unions and INDEX, OFFSET, INDIRECT, CHOOSE and IF as references follow Excel over 45 live formulas | Complete function and argument coverage, dynamic arrays and array formulas (`FormulaArray`), relative R1C1 in INDIRECT and unions for functions beyond the aggregates, errors, circular/iterative calculation, cached-value persistence and names with relative references |
| VBA WorksheetFunction and Evaluate | WorksheetFunction.X and the late-bound Application.X call the cell formula engine for its 75 functions, with arguments, answers and errors converted as 160 live calls made both ways show; Evaluate, `[...]` and Worksheet.Evaluate work out expressions as Excel does over 91 live cases: a Range for cells, blocks whole, error values | Run a function once per item of a range or array given for one value; more functions in the engine; ROW()/COLUMN() without an argument as a one-item array; XLOOKUP's Range from Evaluate |
