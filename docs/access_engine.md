...
* **Jet 4 `.mdb` files** take the writers as `.accdb` files do: a table
  ...
  the workgroup security model, which stays out of scope. ACE refuses
...
* Jet 4 (`.mdb`, version 1) and ACE (versions 2, 3, 5) share the page
  format and are all in scope for reading and writing. Jet 3 (Access 97,
  2 KiB pages) is read but not written: its rows count columns in a byte,
  its text is code page bytes and its index keys collate by other rules,
  so writing it is a second engine rather than a second set of offsets.
...
| 10 | compaction | done for the part that can be done without moving a page. `db.compact()` gives back the run of free pages the file ends with -- what a dropped table or a large delete leaves behind -- and returns how many went. It moves nothing and rewrites no object, so nothing can be lost, and free pages in the middle are left where they are. A live gate drops a 2000-row table, compacts, and has the engine read the file back row for row and then compact it itself. Not this: Access's Compact and Repair, which rebuilds the database, renumbers pages, resets AutoNumber counters and drops deleted rows from the middle |
| 11 | Jet 3 (Access 97) | done for reading. Everything that moved between the versions lives in `_layout.Layout`, so one parser, one row splitter and one set of value decoders read both: the page halves to 2 KiB, a row counts its columns and its variable-column offsets in bytes rather than words, text is stored in the code page page 0 names rather than UTF-16, a name is prefixed by a byte rather than a word, and the definition header, column header, real index and logical index all shrink and move. Every offset was measured, not assumed: the parser checks that what it consumed equals the length the page declares. The live gate has DAO 3.6 build a database with every Jet 3 column type, a memo too long for a 2 KiB page, code page text, a deleted row and four hundred rows over many pages, and pyOpenVBA must agree with the engine cell for cell. Writing is refused at the page store, so no Jet 4 shape can reach a Jet 3 file |
