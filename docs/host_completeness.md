...
| Formatting and layout | Cell formats are complete for Font, Interior, Borders, alignment, protection, NumberFormat and ClearFormats: 161 live probes, an Excel-authored workbook read back value for value, the border storage rules, a live gate for model-written formats, and a stylesheet Excel's for the same edits except one font-order quirk. Row heights, column widths and hidden rows and columns follow Excel on a 96-DPI display: 152 live probes, Excel-authored and foreign files read back, the saved XML Excel's, and a live gate | Row and column formats and whole-row/column formatting, named cell styles (`Range.Style`, `Styles`), conditional formatting, validation and page setup; row heights that fonts set, column AutoFit, and Normal fonts other than Aptos Narrow 11 and Calibri 11 |
