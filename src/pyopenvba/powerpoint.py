...
class PowerPointFile:
    ...
    @classmethod
    def create_new(cls, path: Union[str, Path]) -> "PowerPointFile":
        """
    ...
    def __enter__(self) -> "PowerPointFile":
        return self
    ...
    def vba_project_bytes(self) -> bytes:
        """Return the raw bytes of ``ppt/vbaProject.bin`` (or the whole CFB
        file for legacy ``.ppt``)."""
    ...
    def pull_modules(
        ...
        *,
        ...
        designer, and other modules are written as ``<name>.cls``.
    ...
    def save(
        ...
        *,
        ...
        """
        Save the presentation, applying any pending module edits.

        ``dest`` defaults to the original file path (in-place overwrite).

        Only ``ppt/vbaProject.bin`` is rewritten; every other ZIP entry is
        preserved byte-for-byte.

        Legacy ``.ppt`` (raw CFB) writes the CFB bytes directly.
        ...
        if self._zip is None:
            raise RuntimeError("PowerPointFile is not open.")
